...
PID_FILE="$(get_service_pid_file "$ENVIRONMENT" frontend)"
...
case "$APP_ENV" in
  development)
    FRONTEND_START_SCRIPT="dev:start"
    SHOULD_BUILD=false
    SHOULD_INCLUDE_MODE=true
    ;;
  test|production)
    FRONTEND_START_SCRIPT="preview"
    SHOULD_BUILD=true
    SHOULD_INCLUDE_MODE=false
    ;;
  *)
...
if [ "$SHOULD_INCLUDE_MODE" = true ]; then
  set -- --mode "$APP_ENV" "$@"
fi

FRONTEND_ENV="$APP_ENV" npm run "$FRONTEND_START_SCRIPT" --prefix "$PROJECT_ROOT/Frontend" -- \
  "$@" &
CHILD_PID=$!
wait "$CHILD_PID"
